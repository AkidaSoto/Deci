function [trl,trialinfo] = ABProb_Polymerase(cfg)
...
for j = 1:length(startstopseg)
    ...
    sample{ismember(value,50)} = sample{ismember(value,53)} - 1500;
                    ...
                    if ~isempty(cfg.DT.Displace.Markers)
                        ...
                        for dmrk = 1:length(cfg.DT.Displace.Markers)
                            ...
                            if cfg.DT.Displace.Num > 0
                                ...
                                dmrks{stat,dis,dura} = [nan([-cfg.DT.Displace.Num+dura-1 size(dmrks{stat,dis,dura},2)]); dmrks{stat,dis,dura}(1:end+[-1*cfg.DT.Displace.Num-dura-1])];
                            end
