function [MI_matrix_raw,MI_matrix_surr] = Deci_PACmeg(cfg,data)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for phase = 1:length(phase_freqs)
        [filt] = (squeeze(wavelet_spectrogram(data, Fs, phase_freqs(phase), 3, 0, '')))';
% for phase = 1:length(phase_freqs)
%     try
...
for amp = 1:length(amp_freqs)
    ...
    switch amp_bandw_method
        ...
        case 'maxphase'
            ...
            amp_bandw = 1.5.*max(phase_freqs);
            
        case 'centre_freq'
            if amp == 1
                fprintf('Bandwidth = 2.5* centre amplitude frequency\n')
            end
            amp_bandw = (amp_freqs(amp)/2.5);
    end

        % Filter
[filt] = (squeeze(wavelet_spectrogram(data, Fs, amp_freqs(amp), amp_bandw, 0, '')))';
