...
for i=srtidx(:)'
    ...
    [B,I] = sort([freq.(cfg.widthparam)(i,:,:)],'descend');
    rgb(1:length(I),:) = rgb(I,:);
    
    for j = 1:length(freq.(cfg.colorparam))
        ...
        if ~isnan(B(j))
            ...
            if strcmp(cfg.arrowhead, 'none')
                ...
                set(h, 'EdgeColor', rgb(j,:));
                set(h, 'FaceColor', rgb(j,:)); % for arrowheads
        end
...
if strcmp(cfg.newfigure, 'yes')
  idx = ismember(lay.label,freq.labelcmb);
  lay.pos = lay.pos(idx,:);
