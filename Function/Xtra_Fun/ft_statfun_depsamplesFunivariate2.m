...
within = table(inherent, context, 'VariableNames', {'Inherent' 'Contextual'});
...
if strcmp(cfg.computestat,'yes')
   ...
   for c = 1:size(dat,1)
    ...
    statdat =  reshape(dat(c,:),[max(design(2,:)) max(design(1,:))]);
    statdat = array2table(statdat,'VariableNames',{'x1' 'x2' 'x3' 'x4'});

    rm = fitrm(statdat,'x1-x4~1','WithinDesign',within);
    [ranovatblb] = ranova(rm, 'WithinModel','Inherent*Contextual');
    
    FValues(c,:) = ranovatblb{3:2:end,4};
    s.prob(c,:) = ranovatblb{3:2:end,5};
...
statdat =  reshape(dat(1,:),[max(design(2,:)) max(design(1,:))]);
statdat = array2table(statdat,'VariableNames',{'x1' 'x2' 'x3' 'x4'});

rm = fitrm(statdat,'x1-x4~1','WithinDesign',within);
[ranovatblb] = ranova(rm, 'WithinModel','Inherent*Contextual');

s.dfnum =  ranovatblb{3:2:end,2};
s.dfdenom = ranovatblb{4:2:end,2};

if strcmp(cfg.computecritval,'yes')
  

s.critval = arrayfun(@(a,b) finv(.95,a,b),s.dfnum,s.dfdenom);
