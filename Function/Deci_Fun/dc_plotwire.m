...
for conds = 1:size(Subjects,2)
    ...
    for subj = 1:size(AvgData(:,conds),1)
        ...
        Deci.Plot.Wire = Exist(Deci.Plot.Wire, 'XFreq',false);

        if Deci.Plot.Wire.XFreq
            freq = Segdata{subj,conds}.freq;
            time  = Segdata{subj,conds}.time;
            Segdata{subj,conds}.freq = time;
            Segdata{subj,conds}.time = freq;
            Segdata{subj,conds}.(info.parameter) = permute(Segdata{subj,conds}.(info.parameter), [1 3 2]);
        end
