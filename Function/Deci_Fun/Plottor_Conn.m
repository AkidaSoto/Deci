...
for ConnList = 1:length(Params.List)
    ...
    if ischar(Params.List{ConnList})
        conntype = {Params.List{ConnList}{5}};
    else
        conntype = Params.List{ConnList}{5};
    end
    
    for ConnType = 1:length(conntype)
        ...
        for  subject_list = 1:length(Deci.SubjectList)
            ...
            for Conditions = 1:length(Deci.Plot.CondTitle)
                ...
                load([Deci.Folder.Analysis filesep 'Extra' filesep 'Conn' filesep Deci.SubjectList{subject_list}  filesep Deci.Plot.Lock filesep Deci.Plot.CondTitle{Conditions} filesep strjoin([Params.List{ConnList}(1:4) conntype{ConnType}],'_') '.mat'],'conn');
        ...
        for Conditions = 1:size(Subjects,2)
            for subject_list = 1:size(Subjects,1)
                ...
                if ~strcmpi(Deci.Plot.BslRef,Deci.Plot.Lock)
                    ...
                    load([Deci.Folder.Analysis filesep 'Extra' filesep 'Conn' filesep Deci.SubjectList{subject_list}  filesep Deci.Plot.BslRef filesep Deci.Plot.CondTitle{Conditions} filesep strjoin([Params.List{ConnList}(1:4) conntype{ConnType}],'_') '.mat'],'conn');
