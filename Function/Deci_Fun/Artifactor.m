...
for subject_list = 1:length(Deci.SubjectList)
    ...
    if Deci.Art.do
       ...
       postart.trlnum = data.postart.trlnum;
        ...
        cfg.keepchannel = 'yes';
        tcfg.toilim = [abs(nanmax(locks{1},[],2)/1000)+Deci.Art.crittoilim(1) abs(nanmin(locks{1},[],2)/1000)+Deci.Art.crittoilim(2)];
        cfg.channel = 'all';

        data_rej = ft_rejectvisual(cfg,ft_redefinetrial(tcfg,data_copy));

        
        data.postart.locks = postart.locks(logical(data_rej.saminfo),:);
        data.postart.events = postart.events(logical(data_rej.saminfo),:);
        data.postart.trlnum = postart.trlnum(logical(data_rej.saminfo));
        data.locks = locks;
        data.events = events;
        data.trlnum = trlnum;
